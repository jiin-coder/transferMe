...
# 테스트 환경에서 테스트 먼저
{
  ...
  docker exec python__2__test bash -ce "cd /data/site_projects/python__2/src/ ; git pull origin master"
  docker exec python__2__test bash -ce "cd /data/site_projects/python__2/src/ ; pip install -r requirements/prod.txt"
  docker exec python__2__test bash -ce "cd /data/site_projects/python__2_/src/ ; python manage.py test -v 2 --settings=base.settings.prod 2>&1"
} || {
